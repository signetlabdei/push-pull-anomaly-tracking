...
def run_episode(local_sched_type: int, dist_sched_type: int,
    ...
    for k in std_bar(range(max_num_frame)):
        ...
        if dist_sched_type == 0:
            ...
            scheduled = dist_sched.schedule_maf(Q, k)
        ...
        ### PUSH-BASED SUBFRAME ###
        if local_sched_type == 0:
            outcome = local_sched.schedule(P, scheduled)
            local_state[outcome] = 0
            local_aoii[k, outcome] = 0
            successful_push = outcome[outcome < num_clustered_nodes]
        else:
            if local_sched_type == 1:
                choices = np.random.randint(1, P + 1, num_nodes) * np.asarray(local_aoii[k, :] > 0) * (rng.random(num_nodes) < tx_rate)
            elif local_sched_type == 2:
                choices = np.random.randint(1, P + 1, num_nodes) * np.asarray(local_aoii[k, :] > 0) * (rng.random(num_nodes) < local_sched.rate)
            else:    # local_sched_type == 3:
        ...
        if local_sched_type == 2:
            local_sched.update_rate(outcome)
        elif local_sched_type == 3:
...
if __name__ == '__main__':
    ...
    for schedulers in list(sched_dict.keys()):
        loc_sched, dist_sched = sched_dict[schedulers]
        for load in ['hom', 'het']:
