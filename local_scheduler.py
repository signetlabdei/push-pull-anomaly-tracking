...
class LocalAnomalyScheduler:
    ...
    def __init__(this, N, activation, debug_mode):
        this.N = N
        this.psi = np.ones(N)
        this.lambdas = np.ones(N) * activation
        this.debug_mode = debug_mode

    def schedule(this, P, Q, p_thr):
        if (this.debug_mode):
            print('p', this.psi)
        for threshold in np.arange(0, np.max(this.psi) - 1, 1):
            act_prob = np.zeros(this.N)
            for n in range(this.N):
                act_prob[n] = 1 - np.power(1 - this.lambdas[n], np.max([0, this.psi[n] - threshold]))
            A = np.sum(act_prob > 0)
            ...
            if (coll < p_thr):
                return threshold
        return np.max([0, np.max(this.psi) - 2])

    def update_psi(this, threshold, outcome):
        ...
        if (np.min(outcome) > -0.1):
            ...
            this.psi = np.minimum(this.psi, np.ones(this.N) * threshold)
        # Next time step
        this.psi += 1
