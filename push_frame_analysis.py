...
def main():
    ...
    prob_avg = np.zeros((len(frame_sizes), 5))
    prob_95 = np.zeros((len(frame_sizes), 5))
    prob_99 = np.zeros((len(frame_sizes), 5))
    prob_999 = np.zeros((len(frame_sizes), 5))
    ...
    for r in range(4):
        for pi in range(len(frame_sizes)):
            P = frame_sizes[pi]
            print('Scheduler: ', r, 'Frame size: ', P)
            aoii_hist = np.zeros(M + 1)
            for ep in range(episodes):
                print('Episode: ', ep)
                aoii_hist += run_episode(r, M, P, T, nodes, max_age, rate, p_c, mode, debug_mode)[0] / episodes
            ...
            np.savetxt("push_frame_sota_avg.csv", prob_avg, delimiter=",")
            np.savetxt("push_frame_sota_95.csv", prob_95, delimiter=",")
            np.savetxt("push_frame_sota_99.csv", prob_99, delimiter=",")
            np.savetxt("push_frame_sota_999.csv", prob_999, delimiter=",")
